...
SERVED_IMAGES_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "served_images"
)
...
def wait_for_upscaled_image(image_file, params: dict) -> dict:
    ...
    try:
        ...
        input_filename = (
            f"upscale_input_{timestamp}{os.path.splitext(image_file.filename)[1]}"
        )
        ...
        print(
            f"File size: {os.path.getsize(input_path) if os.path.exists(input_path) else 'N/A'}"
        )
                ...
                if "outputs" in output_data:
                    ...
                    for node_id, node_output in output_data["outputs"].items():
                        if "images" in node_output:
                            ...
                            print(
                                f"Output image exists: {os.path.exists(absolute_image_path)}"
                            )

                            # Copy the output image to served_images directory
                            output_filename = f"upscaled_{timestamp}.png"
                            output_path = os.path.join(
                                SERVED_IMAGES_DIR, output_filename
                            )
                            print(f"Copying to: {output_path}")
                            print(
                                f"Served images dir exists: {os.path.exists(SERVED_IMAGES_DIR)}"
                            )
                            print(
                                f"Served images dir is writable: {os.access(SERVED_IMAGES_DIR, os.W_OK)}"
                            )

                            shutil.copy2(absolute_image_path, output_path)
                            print(
                                f"Copy complete. Output exists: {os.path.exists(output_path)}"
                            )
                            ...
                            if input_path and os.path.exists(input_path):
                                try:
                                    ...
                                    print(
                                        f"Cleaned up temporary input image: {input_path}"
                                    )
                                except Exception as e:
                                    print(
                                        f"Warning: Failed to clean up temporary input image: {e}"
                                    )
                            ...
                            return {
                                ...
                                "data": {
                                    ...
                                    "processing_time": output_data.get(
                                        "execution_time", 0
                                    ),
                                    "original_size": {
        ...
        if input_path and os.path.exists(input_path):
            try:
                ...
                print(
                    f"Warning: Failed to clean up temporary input image after error: {cleanup_error}"
                )
        raise
...
def construct_upscale_workflow(image_path: str, params: dict) -> dict:
    ...
    workflow = {
        ...
        "4": {  # Upscaler Model Loader
            ...
            "inputs": {"model_name": comfy_model_name},
        },
        "5": {  # Image Upscale
            "class_type": "ImageUpscaleWithModel",
            "inputs": {"upscale_model": ["4", 0], "image": ["resize", 0]},
        },
        "6": {  # Save Image
...
def upscale_image():
    ...
    try:
        ...
        if not params_str:
            return jsonify(
                {"status": "error", "message": "No parameters provided"}
            ), 400
        ...
        try:
            ...
            return jsonify(
                {"status": "error", "message": "Invalid parameters format"}
            ), 400

        # Print the request data
        print("\nReceived Request Data:")
        print(json.dumps({"image": image_file.filename, "params": params}, indent=2))

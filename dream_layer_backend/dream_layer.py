...
from dream_layer_backend_utils.random_prompt_generator import (
        ...
        if response.status_code == 200:
            ...
            for filename in models:
                ...
                formatted_models.append(
                    {"id": filename, "name": name, "filename": filename}
                )
        else:
...
def get_upscaler_models_endpoint():
    ...
    formatted = [
        {"id": m, "name": m.replace(".pth", ""), "filename": m} for m in models
    ]
...
def show_in_folder():
    ...
    try:
        ...
        if system == "Darwin":  # macOS
            ...
            return jsonify(
                {"status": "success", "message": f"Opened {filename} in Finder"}
            )
        elif system == "Windows":  # Windows
            subprocess.run(["explorer", "/select,", image_path])
            return jsonify(
                {"status": "success", "message": f"Opened {filename} in File Explorer"}
            )
            ...
            return jsonify(
                {
                    "status": "success",
                    "message": f"Opened directory containing {filename}",
                }
            )
        else:
            return jsonify(

...
def inject_hardcoded_values(custom_workflow, original_workflow):
    ...
    try:
        ...
        if ksampler_node:
            ...
            if (
                positive_input
                and isinstance(positive_input, list)
                and len(positive_input) > 0
            ):
                positive_prompt_node = str(positive_input[0])
            if (
                negative_input
                and isinstance(negative_input, list)
                and len(negative_input) > 0
            ):
        ...
        for node_id, node_data in custom_prompt.items():
            ...
            if "prompt" in inputs and "prompt" in hardcoded_values:
                ...
                logger.info(
                    f"Updated prompt in node {node_id} ({class_type}): {hardcoded_values['prompt']}"
                )
            ...
            if "text" in inputs and "prompt" in hardcoded_values:
                ...
                if node_id == positive_prompt_node:
                    ...
                    logger.info(
                        f"Updated positive text in node {node_id} ({class_type}): {hardcoded_values['prompt']}"
                    )
                elif node_id == negative_prompt_node:
                    inputs["text"] = hardcoded_values.get(
                        "negative_prompt", inputs["text"]
                    )
                    logger.info(
                        f"Updated negative text in node {node_id} ({class_type}): {hardcoded_values.get('negative_prompt', '')}"
                    )
                else:
                    # Fallback: determine based on content or node naming
                    current_text = inputs.get("text", "")
                    if (
                        "negative" in current_text.lower()
                        or "ugly" in current_text.lower()
                        or node_id.endswith("_negative")
                    ):
                        inputs["text"] = hardcoded_values.get(
                            "negative_prompt", current_text
                        )
                        logger.info(
                            f"Updated negative text in node {node_id} ({class_type}) (fallback): {hardcoded_values.get('negative_prompt', current_text)}"
                        )
                    else:
                        inputs["text"] = hardcoded_values.get("prompt", current_text)
                        logger.info(
                            f"Updated positive text in node {node_id} ({class_type}) (fallback): {hardcoded_values.get('prompt', current_text)}"
                        )
            ...
            if "width" in inputs and "width" in hardcoded_values:
                ...
                logger.info(
                    f"Updated width in node {node_id} ({class_type}): {hardcoded_values['width']}"
                )

            if "height" in inputs and "height" in hardcoded_values:
                inputs["height"] = hardcoded_values["height"]
                logger.info(
                    f"Updated height in node {node_id} ({class_type}): {hardcoded_values['height']}"
                )
            ...
            if "ckpt_name" in inputs and "ckpt_name" in hardcoded_values:
                ...
                logger.info(
                    f"Updated ckpt_name in node {node_id} ({class_type}): {hardcoded_values['ckpt_name']}"
                )
            ...
            if "image" in inputs and "input_image_path" in hardcoded_values:
                ...
                logger.info(
                    f"Updated image in node {node_id} ({class_type}): {hardcoded_values['input_image_path']}"
                )
        ...
        for node_id, node_data in original_prompt.items():
            ...
            if class_type == "CheckpointLoaderSimple":
                ...
                if node_id == positive_prompt_node:
                    ...
                    logger.info(
                        f"Extracted positive prompt from node {node_id}: {text}"
                    )
                elif node_id == negative_prompt_node:
                    hardcoded_values["negative_prompt"] = text
                    logger.info(
                        f"Extracted negative prompt from node {node_id}: {text}"
                    )
                else:
                    ...
                    if "negative" in text.lower() or "ugly" in text.lower():
                        hardcoded_values["negative_prompt"] = text
                        logger.info(
                            f"Extracted negative prompt from node {node_id} (fallback): {text}"
                        )
                    else:
                        hardcoded_values["prompt"] = text
                        logger.info(
                            f"Extracted positive prompt from node {node_id} (fallback): {text}"
                        )
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    if key == "prompt" and isinstance(value, str):
                        if (
                            value == "beautiful"
                            and "prompt" in core_generation_settings
                        ):
                            obj[key] = core_generation_settings["prompt"]
                            logger.info(
                                f"Updated prompt at {current_path}: {core_generation_settings['prompt']}"
                            )
                    ...
                    # Handle text fields (for local ComfyUI workflows)
                        ...
                        if (
                            ...
                            obj[key] = core_generation_settings["prompt"]
                            logger.info(
                                f"Updated positive text at {current_path}: {core_generation_settings['prompt']}"
                            )
                        elif (
                            value == "ugly"
                            and "negative_prompt" in core_generation_settings
                        ):
                            obj[key] = core_generation_settings["negative_prompt"]
                            logger.info(
                                f"Updated negative text at {current_path}: {core_generation_settings['negative_prompt']}"
                            )
                    ...
                    # Handle other direct field mappings
                        ...
                        if key == "cfg" and "cfg" in core_generation_settings:
                            ...
                            logger.info(
                                f"Updated {key} at {current_path}: {core_generation_settings['cfg']}"
                            )
                        # Direct field matches
                            ...
                            logger.info(
                                f"Updated {key} at {current_path}: {core_generation_settings[key]}"
                            )
    ...
    try:
        ...
        for node_id, node_data in prompt.items():
            if (
                ...
                and node_data.get("class_type") == "SaveImage"
            ):
...
def update_image_paths_in_workflow(workflow, image_path):
    ...
    try:
        ...
        for node_id, node_data in prompt.items():
            if (
                isinstance(node_data, dict)
                and node_data.get("class_type") == "LoadImage"
            ):
                inputs = node_data.get("inputs", {})
                if "image" in inputs:
                    ...
                    logger.info(
                        f"Updated LoadImage node {node_id}: '{old_image}' -> '{image_path}'"
                    )
                    updated_count += 1

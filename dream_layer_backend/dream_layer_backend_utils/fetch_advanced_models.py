...
def get_controlnet_models() -> List[str]:
    ...
    try:
        ...
        if controlnet_path and is_valid_directory(controlnet_path):
            ...
            logger.info(
                f"Using custom ControlNet directory from settings: {controlnet_dir}"
            )
        else:
            ...
            controlnet_dir = os.path.join(
                current_dir, "ComfyUI", "models", "controlnet"
            )
...
def get_upscaler_models() -> List[str]:
    ...
    try:
        ...
        if upscaler_path and is_valid_directory(upscaler_path):
            ...
            logger.info(
                f"Using custom upscaler directory from settings: {upscaler_dir}"
            )
        else:
            # Fallback to default path
            current_dir = os.path.dirname(
            ...
            upscaler_dir = os.path.join(
                current_dir, "ComfyUI", "models", "upscale_models"
            )

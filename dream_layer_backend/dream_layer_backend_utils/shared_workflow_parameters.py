...
from controlnet import create_test_controlnet_image, save_controlnet_image
    ...
    try:
        ...
        for node_id, node_data in workflow.get("prompt", {}).items():
            if node_data.get("class_type") == "KSampler":
                ...
                print(
                    f"🎲 Incremented KSampler seed: {current_seed} -> {current_seed + increment}"
                )
                return workflow
        ...
        for node_id, node_data in workflow.get("prompt", {}).items():
            if node_data.get("class_type") in closed_source_nodes:
                ...
                print(
                    f"🎲 Incremented {node_data.get('class_type')} seed: {current_seed} -> {new_seed}"
                )
                return workflow
    ...
    try:
        ...
        for node_id, node_data in prompt.items():
            if node_data.get("class_type") == "LoraLoader":
                ...
                print(
                    f"Updated LoRA: {lora_name} (model: {strength_model}, clip: {strength_clip})"
                )
                break
...
def inject_controlnet_parameters(workflow, controlnet_data):
    ...
    try:
        ...
        if not controlnet_data.get("enabled", False) or not controlnet_data.get(
            "units"
        ):
        ...
        for node_id, node_data in prompt.items():
            if node_data.get("class_type") == "SetUnionControlNetType":
                if unit.get("control_type"):
                    ...
                    union_type = control_type_mapping.get(
                        unit["control_type"], "openpose"
                    )
        ...
        if input_image_value is not None and input_image_value != "":
            print(
                f"🎯 Processing ControlNet image for unit {unit.get('unit_index', 0)}"
            )
            ...
            if isinstance(input_image, str):
                ...
                if (
                    not input_image.startswith("data:image")
                    and not input_image.startswith("/9j/")
                    and len(input_image) < 1000
                ):
                    ...
                    saved_filename = save_controlnet_image(
                        input_image, unit.get("unit_index", 0)
                    )
            ...
            if saved_filename:
                ...
                print("🔍 Looking for LoadImage node in workflow...")
                print("🔍 Available nodes:")
                for node_id, node_data in prompt.items():
                    print(
                        f"   Node {node_id}: {node_data.get('class_type', 'Unknown')}"
                    )
                ...
                print(
                    "❌ Failed to process ControlNet input image, creating test image"
                )
...
def inject_face_restoration_parameters(workflow, face_restoration_data):
    ...
    try:
        ...
        fidelity_weight = (
            codeformer_weight if model_type == "codeformer" else gfpgan_weight
        )
...
def inject_hires_fix_parameters(workflow, hires_fix_data):
    ...
    try:
        ...
        prompt[upscaler_node_id] = {
            ...
            "inputs": {
                ...
                **(
                    ...
                    if upscale_method == "upscale-by"
                    else {"width": resize_width, "height": resize_height}
                ),
            },
        }
        ...
        prompt[hires_vae_decode_node_id] = {
            ...
            "inputs": {"samples": [hires_ksampler_node_id, 0], "vae": ["4", 2]},
        }

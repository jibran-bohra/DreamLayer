...
def process_controlnet_images(
    controlnet_data: Dict[str, Any], comfy_input_dir: str
) -> Dict[str, Any]:
    """
    ...
    for i, unit in enumerate(units):
        if unit.get("input_image") and unit.get("enabled"):
            try:
                ...
                image_path = process_controlnet_image(
                    unit["input_image"], comfy_input_dir, f"controlnet_{i}"
                )
                units[i]["input_image_path"] = image_path
                logger.info(f"Processed ControlNet image for unit {i}: {image_path}")
            except Exception as e:
                logger.error(
                    f"Error processing ControlNet image for unit {i}: {str(e)}"
                )
...
def inject_controlnet_into_workflow(
    workflow: Dict[str, Any], controlnet_data: Dict[str, Any], comfy_input_dir: str
) -> Dict[str, Any]:
    """
    ...
    next_node_id = str(
        max([int(k) for k in workflow["prompt"].keys() if k.isdigit()]) + 1
    )
    ...
    logger.info(
        f"Injected ControlNet into workflow with unit: {unit.get('control_type', 'unknown')}"
    )
    return workflow

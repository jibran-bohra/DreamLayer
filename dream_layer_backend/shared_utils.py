...
def wait_for_image(
    prompt_id: str, save_node_id: str = "9", max_wait_time: int = 300
) -> List[Dict[str, Any]]:
    """
...
def send_to_comfyui(workflow: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for i in range(iterations):
            ...
            current_workflow = (
                increment_seed_in_workflow(copy.deepcopy(workflow), i)
                if i > 0
                else workflow
            )
            ...
            if response.status_code == 200:
                ...
                error_msg = (
                    f"ComfyUI server error: {response.status_code} - {response.text}"
                )
                print(error_msg)

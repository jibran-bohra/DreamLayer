...
def handle_img2img():
    ...
    try:
        ...
        data = request.json
        logger.info(
            "Received img2img request with data: %s",
            {
                **data,
        ...
        try:
            ...
            # Log image details
            logger.info(
                "Input image saved as: %s, format=%s, size=%s, mode=%s",
                temp_filename,
                image.format,
                image.size,
                image.mode,
            ...
            return jsonify(
                {"status": "error", "message": f"Invalid input image: {str(e)}"}
            ), 400
        ...
        if "error" in comfy_response:
            return jsonify({"status": "error", "message": comfy_response["error"]}), 500

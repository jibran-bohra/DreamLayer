...
COMMON_PARAMETERS = {
    "Seed": (IO.INT, {"default": 0, "min": 0, "max": 65535, "display": "number"}),
    "Material_Type": (IO.COMBO, {"options": ["PBR", "Shaded"], "default": "PBR"}),
    "Polygon_count": (
        IO.COMBO,
        {
...
class Rodin3DAPI:
    ...
    def check_rodin_status(self, response: Rodin3DCheckStatusResponse) -> str:
        ...
        if has_failed:
            logging.error(
                f"[ Rodin3D API - CheckStatus ] Generate Failed: {status_list}, Please try again."
            )
        ...
        operation = SynchronousOperation(
            ...
            files=[
                (
                    ...
                    else self.tensor_to_filelike(image),
                )
                for image in images
    ...
    def DownLoadFiles(self, Url_List):
        ...
        for Item in Url_List.list:
            ...
            logging.info(
                f"[ Rodin3D API - download_files ] Downloading file: {file_path}"
            )
            max_retries = 5
            for attempt in range(max_retries):
                try:
                    ...
                    logging.info(
                        f"[ Rodin3D API - download_files ] Error downloading {file_path}:{e}"
                    )
                    if attempt < max_retries - 1:
                        ...
                        logging.info(
                            f"[ Rodin3D API - download_files ] Failed to download {file_path} after {max_retries} attempts."
                        )
...
class Rodin3D_Regular(Rodin3DAPI):
    ...
    def api_call(self, Images, Seed, Material_Type, Polygon_count, **kwargs):
        tier = "Regular"
        ...
        task_uuid, subscription_key = self.CreateGenerateTask(
            images=m_images,
            seed=Seed,
...
class Rodin3D_Sketch(Rodin3DAPI):
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                "Seed": (
                    IO.INT,
                    {"default": 0, "min": 0, "max": 65535, "display": "number"},
                )
            },
            "hidden": {
                "auth_token": "AUTH_TOKEN_COMFY_ORG",
    ...
    def api_call(self, Images, Seed, **kwargs):
        tier = "Sketch"
        num_images = Images.shape[0]
        m_images = []
        for i in range(num_images):
